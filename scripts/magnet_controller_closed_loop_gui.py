"""
GUI for visualizing and tuning closed-loop control gains (formerly PID gains) of the magnetic field controller.

Features:
- Real-time plotting of control terms (P, I, D, torque, error)
- Interactive sliders to update gains per axis
- Target magnetic direction input
- Live sensor data acquisition and logging
- Control current transmission to MiniMACS6 via Arduino

Recommended usage: Tune gains axis by axis, starting with P only.
"""
...
class MagneticFieldControllerApp(tk.Frame):
    ...
    def create_plots(self):
        ...
        plot_labels = ["|P(t)|", "|I(t)|", "|D(t)|", "|Torque(t)|", "|Error(t)|"]
    ...
    def create_input_frames(self):
        ...
        # Closed-loop gain sliders
        gains_frame = tk.LabelFrame(self, text="Closed-Loop Gains", padx=10, pady=5)
        gains_frame.grid(row=1, column=1, columnspan=2, sticky="new", pady=10)

        self.kp_slider_x = self._create_slider(gains_frame, "Kp X", 0, default_value=1300)
        self.ki_slider_x = self._create_slider(gains_frame, "Ki X", 1, default_value=1300)
        self.kd_slider_x = self._create_slider(gains_frame, "Kd X", 2, default_value=65, resolution=1)

        self.kp_slider_y = self._create_slider(gains_frame, "Kp Y", 3, default_value=1250)
        self.ki_slider_y = self._create_slider(gains_frame, "Ki Y", 4, default_value=1250)
        self.kd_slider_y = self._create_slider(gains_frame, "Kd Y", 5, default_value=62, resolution=1)

        self.kp_slider_z = self._create_slider(gains_frame, "Kp Z", 6, default_value=1000)
        self.ki_slider_z = self._create_slider(gains_frame, "Ki Z", 7, default_value=1000)
        self.kd_slider_z = self._create_slider(gains_frame, "Kd Z", 8, default_value=50, resolution=1)

        self.lambda_slider = self._create_slider(gains_frame, "Lambda", 9, to=10, default_value=0.0, resolution=0.1)

        tk.Button(gains_frame, text="Set Gains", command=self.set_gains).grid(row=10, column=0, columnspan=6, pady=10)
    ...
    def create_control_buttons(self):
        ...
        tk.Button(ctrl_frame, text="Reset", command=self.reset_data).pack(side="left", padx=10)
    ...
    def _create_slider(self, parent, label, row, to=20000, default_value=0, resolution=25.0):
    ...
    def set_target(self):
        try:
            ...
            if vec_norm < 1e-7:
                raise ValueError("Target vector magnitude is too small.")
    ...
    def set_gains(self):
        ...
        self.controller.set_closed_loop_gains(gains)
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Magnetic Field Controller - Closed-Loop Gain Tuner")
