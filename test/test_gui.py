...
class MagneticFieldControllerApp(tk.Frame):
    def __init__(self, master=None):
        ...
        self.data_log_columns = [
            ...
            'angular_error_degrees', 'p_term_mag', 'i_term_mag', 'd_term_mag',
    ...
    def update_plots(self):
        if self.plot_updating.get():
            ...
            dummy_vals = np.random.rand(5) * 100  # simulate data

            for i in range(5):
                self.plot_data[i] = np.roll(self.plot_data[i], -1)
                self.plot_data[i, -1] = dummy_vals[i]
                self.lines[i].set_ydata(self.plot_data[i])
                self.lines[i].set_xdata(self.x_data)
                self.axes[i].relim()
                self.axes[i].autoscale_view()
            ...
            # Log data (example with dummy values)
            new_row = {
                'time': current_time,
                'm_x': 0.1, 'm_y': 0.2, 'm_z': 0.3,
                'm_target_x': self.target_vec[0],
                'm_target_y': self.target_vec[1],
                'm_target_z': self.target_vec[2],
                'angular_error_degrees': 5,
                'p_term_mag': dummy_vals[0],
                'i_term_mag': dummy_vals[1],
                'd_term_mag': dummy_vals[2],
                'KP_x': self.kp_slider_x.get(),
                'KI_x': self.ki_slider_x.get(),
                'KD_x': self.kd_slider_x.get(),
                'KP_y': self.kp_slider_y.get(),
                ...
                'KI_z': self.ki_slider_z.get(),
                'KD_z': self.kd_slider_z.get(),
                'i_x': 0.1, 'i_y': 0.2, 'i_z': 0.3
            }
            self.data_log.loc[len(self.data_log)] = new_row
